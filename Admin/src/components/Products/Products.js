...
export default function Products({ showToast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const mappedVariants = variants.map((v, idx) => {
      // attributes may be missing on SKUs generated by VariantBuilder (they only have combo)
      const rawAttrs = v.attributes?.length
        ? v.attributes
        : (v.combo?.map(c => ({ key: c.key, value: c.value, customValue: '' })) || []);

      const variantName = rawAttrs
        .map(a => {
          const val = a.value === 'Custom' ? (a.customValue || 'Custom') : a.value;
          return val ? (a.key ? `${a.key}: ${val}` : val) : '';
        })
        .filter(Boolean)
        .join(' · ') || v.variantName || 'Default';
